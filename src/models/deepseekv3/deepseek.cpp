...
__C void
inferBatch(struct DeepseekV3Model *model,
           const uint32_t *tokens, uint32_t ntok,
           const uint32_t *req_lens, uint32_t nreq, const uint32_t *req_pos,
           struct KVCache **kv_caches,
           const float *temperature, const uint32_t *topk, const float *topp,
...
__C void
forwardBatch(struct DeepseekV3Model *model,
             const uint32_t *tokens, uint32_t ntok,
             const uint32_t *req_lens, uint32_t nreq, const uint32_t *req_pos,
             struct KVCache **kv_caches,
             void *logits) {
...
DeepseekV3Model::DeepseekV3Model(const DeepseekV3Meta *_meta, const DeepseekV3Weights *weights, infiniDevice_t device_, std::vector<int> device_ids) : meta(*_meta) {
...
createDeepseekV3Model(const DeepseekV3Meta *meta,
    ...
    DeepseekV3Model *model = new DeepseekV3Model(meta, weights, device, device_ids);
    return model;
}

__C void destroyDeepseekV3Model(struct DeepseekV3Model *model) {
